...
class JaizBank extends StatefulWidget {
  const JaizBank({Key? key}) : super(key: key);

  @override
  _ZenithBank createState() => _ZenithBank();
}

class _ZenithBank extends State<JaizBank> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0.0,
        bottomOpacity: 0.0,
        title: Text("Jaiz Bank"),
      ),
      body: ListView.builder(
          scrollDirection: Axis.vertical,
          shrinkWrap: true,
          itemCount: jaizTransactionList.length,
          itemBuilder: (context, index) {
            return GestureDetector(
              onTap: () {
                if (jaizTransactionList[index].inputType == 'balance') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${jaizTransactionList[index].transactionUssd}');
                } else if (jaizTransactionList[index].inputType == 'customer') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${jaizTransactionList[index].transactionUssd}');
                } else if (jaizTransactionList[index].inputType == '0') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${jaizTransactionList[index].transactionUssd}');
                } else {
                  openDialog(index);
              ...
              child: Card(
                ...
                child: ListTile(
                  ...
                  subtitle: Text(
                    "${jaizTransactionList[index].bankUssd}",
                    style: TextStyle(color: Colors.white),
                  ),
                ),
              ),
  ...
  Future openDialog(index) async => showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('${jaizTransactionList[index].transactionName}'),
          content: showDialogTextField(jaizTransactionList[index].inputType),
          actions: [
            Form(
              child: Row(
                ...
                children: [
                  TextButton(
                      onPressed: () {
                        submitUssd(jaizTransactionList[index].inputType,
                            jaizTransactionList[index].transactionUssd);
                      },
                      child: Row(
                        children: [
