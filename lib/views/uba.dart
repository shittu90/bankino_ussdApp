...
class UbaBank extends StatefulWidget {
  const UbaBank({Key? key}) : super(key: key);

  @override
  _ZenithBank createState() => _ZenithBank();
}

class _ZenithBank extends State<UbaBank> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.builder(
          ...
          shrinkWrap: true,
          itemCount: ubaTransactionList.length,
          itemBuilder: (context, index) {
            return GestureDetector(
              onTap: () {
                if (ubaTransactionList[index].inputType == 'balance') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${ubaTransactionList[index].transactionUssd}');
                } else if (ubaTransactionList[index].inputType == 'customer') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${ubaTransactionList[index].transactionUssd}');
                } else if (ubaTransactionList[index].inputType == '0') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${ubaTransactionList[index].transactionUssd}');
                } else {
                  openDialog(index);
              ...
              child: Card(
                ...
                child: ListTile(
                  title: Text('${ubaTransactionList[index].transactionName}',
                      style: TextStyle(color: Colors.white)),
                  leading: CircleAvatar(
                    radius: 30.0,
                    backgroundImage: AssetImage('images/ubaavater.png'),
                  ...
                  subtitle: Text(
                    "${ubaTransactionList[index].bankUssd}",
                    style: TextStyle(color: Colors.white),
                  ),
                ),
              ),
  ...
  Future openDialog(index) async => showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('${ubaTransactionList[index].transactionName}'),
          content: showDialogTextField(ubaTransactionList[index].inputType),
          actions: [
            Form(
              child: Row(
                ...
                children: [
                  TextButton(
                      onPressed: () {
                        submitUssd(ubaTransactionList[index].inputType,
                            ubaTransactionList[index].transactionUssd);
                      },
                      child: Row(
                        children: [
