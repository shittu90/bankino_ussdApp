...
class SterlingBank extends StatefulWidget {
  const SterlingBank({Key? key}) : super(key: key);

  @override
  _ZenithBank createState() => _ZenithBank();
}

class _ZenithBank extends State<SterlingBank> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.builder(
          ...
          shrinkWrap: true,
          itemCount: sterlingTransactionList.length,
          itemBuilder: (context, index) {
            return GestureDetector(
              onTap: () {
                if (sterlingTransactionList[index].inputType == 'balance') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${sterlingTransactionList[index].transactionUssd}');
                } else if (sterlingTransactionList[index].inputType ==
                    'customer') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${sterlingTransactionList[index].transactionUssd}');
                } else if (sterlingTransactionList[index].inputType == '0') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${sterlingTransactionList[index].transactionUssd}');
                } else {
                  openDialog(index);
              ...
              child: Card(
                ...
                child: ListTile(
                  title: Text(
                      '${sterlingTransactionList[index].transactionName}',
                      style: TextStyle(color: Colors.white)),
                  leading: CircleAvatar(
                    radius: 30.0,
                    backgroundImage: AssetImage('images/sterlingavater.png'),
                  ...
                  subtitle: Text(
                    "${sterlingTransactionList[index].bankUssd}",
                    style: TextStyle(color: Colors.white),
                  ),
                ),
              ),
  ...
  Future openDialog(index) async => showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('${sterlingTransactionList[index].transactionName}'),
          content:
              showDialogTextField(sterlingTransactionList[index].inputType),
          actions: [
            Form(
              child: Row(
                ...
                children: [
                  TextButton(
                      onPressed: () {
                        submitUssd(sterlingTransactionList[index].inputType,
                            sterlingTransactionList[index].transactionUssd);
                      },
                      child: Row(
                        children: [
