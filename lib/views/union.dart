...
class UnionBank extends StatefulWidget {
  const UnionBank({Key? key}) : super(key: key);

  @override
  _ZenithBank createState() => _ZenithBank();
}

class _ZenithBank extends State<UnionBank> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0.0,
        bottomOpacity: 0.0,
        title: Text("Union Bank"),
      ),
      body: ListView.builder(
          scrollDirection: Axis.vertical,
          shrinkWrap: true,
          itemCount: unionTransactionList.length,
          itemBuilder: (context, index) {
            return GestureDetector(
              onTap: () {
                if (unionTransactionList[index].inputType == 'balance') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${unionTransactionList[index].transactionUssd}');
                } else if (unionTransactionList[index].inputType ==
                    'customer') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${unionTransactionList[index].transactionUssd}');
                } else if (unionTransactionList[index].inputType == '0') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${unionTransactionList[index].transactionUssd}');
                } else {
                  openDialog(index);
              ...
              child: Card(
                ...
                child: ListTile(
                  title: Text('${unionTransactionList[index].transactionName}',
                      style: TextStyle(color: Colors.white)),
                  leading: CircleAvatar(
                    radius: 30.0,
                    backgroundImage: AssetImage('images/unionlogo.png'),
                  ...
                  subtitle: Text(
                    "${unionTransactionList[index].bankUssd}",
                    style: TextStyle(color: Colors.white),
                  ),
                ),
              ),
  ...
  Future openDialog(index) async => showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('${unionTransactionList[index].transactionName}'),
          content: showDialogTextField(unionTransactionList[index].inputType),
          actions: [
            Form(
              child: Row(
                ...
                children: [
                  TextButton(
                      onPressed: () {
                        submitUssd(unionTransactionList[index].inputType,
                            unionTransactionList[index].transactionUssd);
                      },
                      child: Row(
                        children: [
