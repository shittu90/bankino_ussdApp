...
class KeyStoneBank extends StatefulWidget {
  const KeyStoneBank({Key? key}) : super(key: key);

  @override
  _ZenithBank createState() => _ZenithBank();
}

class _ZenithBank extends State<KeyStoneBank> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.builder(
          ...
          shrinkWrap: true,
          itemCount: keystoneTransactionList.length,
          itemBuilder: (context, index) {
            return GestureDetector(
              onTap: () {
                if (keystoneTransactionList[index].inputType == 'balance') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${keystoneTransactionList[index].transactionUssd}');
                } else if (keystoneTransactionList[index].inputType ==
                    'customer') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${keystoneTransactionList[index].transactionUssd}');
                } else if (keystoneTransactionList[index].inputType == '0') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${keystoneTransactionList[index].transactionUssd}');
                } else {
                  openDialog(index);
              ...
              child: Card(
                ...
                child: ListTile(
                  ...
                  leading: CircleAvatar(
                    radius: 30.0,
                    backgroundImage: AssetImage('images/keystonelogo.png'),
                  ...
                  subtitle: Text(
                    "${keystoneTransactionList[index].bankUssd}",
                    style: TextStyle(color: Colors.white),
                  ),
                ),
              ),
  ...
  Future openDialog(index) async => showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('${keystoneTransactionList[index].transactionName}'),
          content:
              showDialogTextField(keystoneTransactionList[index].inputType),
          actions: [
            Form(
              child: Row(
                ...
                children: [
                  TextButton(
                      onPressed: () {
                        submitUssd(keystoneTransactionList[index].inputType,
                            keystoneTransactionList[index].transactionUssd);
                      },
                      child: Row(
                        children: [
