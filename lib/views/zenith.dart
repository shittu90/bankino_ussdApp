...
class ZenithBank extends StatefulWidget {
  const ZenithBank({Key? key}) : super(key: key);

  @override
  _ZenithBank createState() => _ZenithBank();
}

class _ZenithBank extends State<ZenithBank> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.builder(
          ...
          shrinkWrap: true,
          itemCount: zenithTransactionList.length,
          itemBuilder: (context, index) {
            return GestureDetector(
              onTap: () {
                if (zenithTransactionList[index].inputType == 'balance') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${zenithTransactionList[index].transactionUssd}');
                } else if (zenithTransactionList[index].inputType ==
                    'customer') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${zenithTransactionList[index].transactionUssd}');
                } else if (zenithTransactionList[index].inputType == '0') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${zenithTransactionList[index].transactionUssd}');
                } else {
                  openDialog(index);
              ...
              child: Card(
                ...
                child: ListTile(
                  ...
                  subtitle: Text(
                    "${zenithTransactionList[index].bankUssd}",
                    style: TextStyle(color: Colors.white),
                  ),
                ),
              ),
  ...
  Future openDialog(index) async => showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('${zenithTransactionList[index].transactionName}'),
          content: showDialogTextField(zenithTransactionList[index].inputType),
          actions: [
            Form(
              child: Row(
                ...
                children: [
                  TextButton(
                      onPressed: () {
                        submitUssd(zenithTransactionList[index].inputType,
                            zenithTransactionList[index].transactionUssd);
                      },
                      child: Row(
                        children: [
