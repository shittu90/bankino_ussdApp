...
class StanbicBank extends StatefulWidget {
  const StanbicBank({Key? key}) : super(key: key);

  @override
  _ZenithBank createState() => _ZenithBank();
}

class _ZenithBank extends State<StanbicBank> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0.0,
        bottomOpacity: 0.0,
        title: Text("Stabic IBTC Bank"),
      ),
      body: ListView.builder(
          scrollDirection: Axis.vertical,
          shrinkWrap: true,
          itemCount: stanbicTransactionList.length,
          itemBuilder: (context, index) {
            return GestureDetector(
              onTap: () {
                if (stanbicTransactionList[index].inputType == 'balance') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${stanbicTransactionList[index].transactionUssd}');
                } else if (stanbicTransactionList[index].inputType ==
                    'customer') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${stanbicTransactionList[index].transactionUssd}');
                } else if (stanbicTransactionList[index].inputType == '0') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${stanbicTransactionList[index].transactionUssd}');
                } else {
                  openDialog(index);
              ...
              child: Card(
                ...
                child: ListTile(
                  ...
                  subtitle: Text(
                    "${stanbicTransactionList[index].bankUssd}",
                    style: TextStyle(color: Colors.white),
                  ),
                ),
              ),
  ...
  Future openDialog(index) async => showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('${stanbicTransactionList[index].transactionName}'),
          content: showDialogTextField(stanbicTransactionList[index].inputType),
          actions: [
            Form(
              child: Row(
                ...
                children: [
                  TextButton(
                      onPressed: () {
                        submitUssd(stanbicTransactionList[index].inputType,
                            stanbicTransactionList[index].transactionUssd);
                      },
                      child: Row(
                        children: [
