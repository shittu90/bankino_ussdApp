...
class AccessBank extends StatefulWidget {
  const AccessBank({Key? key}) : super(key: key);

  @override
  _ZenithBank createState() => _ZenithBank();
}

class _ZenithBank extends State<AccessBank> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.builder(
          ...
          shrinkWrap: true,
          itemCount: accessTransactionList.length,
          itemBuilder: (context, index) {
            return GestureDetector(
              onTap: () {
                if (accessTransactionList[index].inputType == 'balance') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${accessTransactionList[index].transactionUssd}');
                } else if (accessTransactionList[index].inputType ==
                    'customer') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${accessTransactionList[index].transactionUssd}');
                } else if (accessTransactionList[index].inputType == '0') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${accessTransactionList[index].transactionUssd}');
                } else {
                  openDialog(index);
              ...
              child: Card(
                ...
                child: ListTile(
                  title: Text('${accessTransactionList[index].transactionName}',
                      style: TextStyle(color: Colors.white)),
                  leading: CircleAvatar(
                    radius: 30.0,
                    backgroundImage: AssetImage('images/access bank.png'),
                  ...
                  subtitle: Text(
                    "${accessTransactionList[index].bankUssd}",
                    style: TextStyle(color: Colors.white),
                  ),
                ),
              ),
  ...
  Future openDialog(index) async => showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('${accessTransactionList[index].transactionName}'),
          content: showDialogTextField(accessTransactionList[index].inputType),
          actions: [
            Form(
              child: Row(
                ...
                children: [
                  TextButton(
                      onPressed: () {
                        submitUssd(accessTransactionList[index].inputType,
                            accessTransactionList[index].transactionUssd);
                      },
                      child: Row(
                        children: [
