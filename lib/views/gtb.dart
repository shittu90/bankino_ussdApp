...
class GtBank extends StatefulWidget {
  const GtBank({Key? key}) : super(key: key);

  @override
  _ZenithBank createState() => _ZenithBank();
}

class _ZenithBank extends State<GtBank> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0.0,
        bottomOpacity: 0.0,
        title: Text("Guaranty Trust Bank"),
      ),
      body: ListView.builder(
          scrollDirection: Axis.vertical,
          shrinkWrap: true,
          itemCount: gtTransactionList.length,
          itemBuilder: (context, index) {
            return GestureDetector(
              onTap: () {
                if (gtTransactionList[index].inputType == 'balance') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${gtTransactionList[index].transactionUssd}');
                } else if (gtTransactionList[index].inputType == 'customer') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${gtTransactionList[index].transactionUssd}');
                } else if (gtTransactionList[index].inputType == '0') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${gtTransactionList[index].transactionUssd}');
                } else {
                  openDialog(index);
              ...
              child: Card(
                ...
                child: ListTile(
                  ...
                  leading: CircleAvatar(
                    radius: 30.0,
                    backgroundImage: AssetImage('images/gtlogo.png'),
                  ...
                  subtitle: Text(
                    "${gtTransactionList[index].bankUssd}",
                    style: TextStyle(color: Colors.white),
                  ),
                ),
              ),
  ...
  Future openDialog(index) async => showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('${gtTransactionList[index].transactionName}'),
          content: showDialogTextField(gtTransactionList[index].inputType),
          actions: [
            Form(
              child: Row(
                ...
                children: [
                  TextButton(
                      onPressed: () {
                        submitUssd(gtTransactionList[index].inputType,
                            gtTransactionList[index].transactionUssd);
                      },
                      child: Row(
                        children: [
