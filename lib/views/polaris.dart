...
class PolarisBank extends StatefulWidget {
  const PolarisBank({Key? key}) : super(key: key);

  @override
  _ZenithBank createState() => _ZenithBank();
}

class _ZenithBank extends State<PolarisBank> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0.0,
        bottomOpacity: 0.0,
        title: Text("Polaris Bank"),
      ),
      body: ListView.builder(
          scrollDirection: Axis.vertical,
          shrinkWrap: true,
          itemCount: polarisTransactionList.length,
          itemBuilder: (context, index) {
            return GestureDetector(
              onTap: () {
                if (polarisTransactionList[index].inputType == 'balance') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${polarisTransactionList[index].transactionUssd}');
                } else if (polarisTransactionList[index].inputType ==
                    'customer') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${polarisTransactionList[index].transactionUssd}');
                } else if (polarisTransactionList[index].inputType == '0') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${polarisTransactionList[index].transactionUssd}');
                } else {
                  openDialog(index);
              ...
              child: Card(
                ...
                child: ListTile(
                  title: Text(
                      '${polarisTransactionList[index].transactionName}',
                      style: TextStyle(color: Colors.white)),
                  leading: CircleAvatar(
                    radius: 30.0,
                    backgroundImage: AssetImage('images/polarislogo.png'),
                  ...
                  subtitle: Text(
                    "${polarisTransactionList[index].bankUssd}",
                    style: TextStyle(color: Colors.white),
                  ),
                ),
              ),
  ...
  Future openDialog(index) async => showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('${polarisTransactionList[index].transactionName}'),
          content: showDialogTextField(polarisTransactionList[index].inputType),
          actions: [
            Form(
              child: Row(
                ...
                children: [
                  TextButton(
                      onPressed: () {
                        submitUssd(polarisTransactionList[index].inputType,
                            polarisTransactionList[index].transactionUssd);
                      },
                      child: Row(
                        children: [
