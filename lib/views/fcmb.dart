...
class FcmbBank extends StatefulWidget {
  const FcmbBank({Key? key}) : super(key: key);

  @override
  _ZenithBank createState() => _ZenithBank();
}

class _ZenithBank extends State<FcmbBank> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0.0,
        bottomOpacity: 0.0,
        title: Text("FCMB Bank"),
      ),
      body: ListView.builder(
          scrollDirection: Axis.vertical,
          shrinkWrap: true,
          itemCount: fcmbTransactionList.length,
          itemBuilder: (context, index) {
            return GestureDetector(
              onTap: () {
                if (fcmbTransactionList[index].inputType == 'balance') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${fcmbTransactionList[index].transactionUssd}');
                } else if (fcmbTransactionList[index].inputType == 'customer') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${fcmbTransactionList[index].transactionUssd}');
                } else if (fcmbTransactionList[index].inputType == '0') {
                  FlutterPhoneDirectCaller.callNumber(
                      '${fcmbTransactionList[index].transactionUssd}');
                } else {
                  openDialog(index);
              ...
              child: Card(
                ...
                child: ListTile(
                  title: Text('${fcmbTransactionList[index].transactionName}',
                      style: TextStyle(color: Colors.white)),
                  leading: CircleAvatar(
                    radius: 30.0,
                    backgroundImage: AssetImage('images/fcmblogo.png'),
                  ...
                  subtitle: Text(
                    "${fcmbTransactionList[index].bankUssd}",
                    style: TextStyle(color: Colors.white),
                  ),
                ),
              ),
  ...
  Future openDialog(index) async => showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('${fcmbTransactionList[index].transactionName}'),
          content: showDialogTextField(fcmbTransactionList[index].inputType),
          actions: [
            Form(
              child: Row(
                ...
                children: [
                  TextButton(
                      onPressed: () {
                        submitUssd(fcmbTransactionList[index].inputType,
                            fcmbTransactionList[index].transactionUssd);
                      },
                      child: Row(
                        children: [
